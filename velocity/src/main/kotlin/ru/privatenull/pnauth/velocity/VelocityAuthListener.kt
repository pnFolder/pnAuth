...
class VelocityAuthListener(
    ...
    fun onServerPostConnect(event: ServerPostConnectEvent) {
        ...
        if (!proxySettings.isAuthServer(current) || auth.isAuthenticated(player.uniqueId)) return
    ...
    fun onKickedFromServer(event: KickedFromServerEvent) {
        if (!proxySettings.isAuthServer(event.server.serverInfo.name)) return
    ...
    private fun authServer(): RegisteredServer? {
        if (embeddedAuthServer != null && proxySettings.isAuthServer(embeddedAuthServer.serverInfo.name)) {
        ...
        val selected = balancer.selectServer(targets, proxyAdapter).orElse(targets.first())
    ...
    private fun resolveBackend(player: Player): RegisteredServer? {
        ...
        if (virtualHost != null) {
            val forced = proxySettings.forcedHosts[virtualHost.hostString.lowercase(Locale.ROOT)]
            if (!forced.isNullOrBlank()) return proxy.getServer(forced).orElse(null)
        }
        val targets = proxySettings.getEffectiveBackendServers()
        if (targets.isEmpty()) return null
        val balancer = ServerBalancerFactory.create(
            proxySettings.balancerMode,
            proxySettings.maxPlayersPerServer,
