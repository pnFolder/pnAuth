...
Успешная перезагрузка пересоздаёт сервисы, зависящие от настроек, но сохраняет уже работающий
встроенный PicoLimbo. Изменение provider, адреса или порта limbo требует перезапуска прокси.
...
Конфигурация описана в `PnAuthYamlConfig` и генерируется Elytrium Serializer. На первом запуске появляется полный `config.yml` с комментариями. После создания файл принадлежит администратору: pnAuth не перезаписывает и не форматирует его ни при запуске, ни при `/auth reload`. Совместимость со старыми схемами применяется только в памяти.

Загрузка разделена на независимые этапы: стабильный снимок файла, миграция снимка, десериализация временной копии и построение проверенной runtime-конфигурации. Новый файл создаётся через временный документ и атомарную установку; при ошибке исходный `config.yml` и уже работающая конфигурация не изменяются.
...
messages:
  ...
  # MINI_MESSAGE также поддерживает интерактивные теги pnAuth.
  format: MINI_MESSAGE
...
`messages/messages_ru.yml` и `messages/messages_en.yml` создаются автоматически. Меняйте значения и оформление свободно. При обновлении отсутствующие ключи берутся из встроенного перевода. При одноразовой миграции старых раздельных `dialog.error`, `dialog.retry` и `dialog.retry_hover` создаётся файл `.bak`, после чего они объединяются в один `dialog.error`.

Командные ответы (`usage`, `prompt`, `result`, `status`, `access`, `admin`), сообщения CAPTCHA и причины отключения также находятся в этих файлах. Стандартные белые строки автоматически получают редактируемый MiniMessage-префикс и цвет; перед одноразовым обновлением существующего файла создаётся `.bak`. Paper, Folia, BungeeCord и Velocity используют один и тот же компонентный конвейер — платформенные адаптеры не отправляют форматированные строки через `sendMessage(String)`.

### MiniMessage и действия pnAuth

Рекомендуемый формат сообщений — `MINI_MESSAGE`. Он поддерживает стандартные теги Adventure MiniMessage: цвета, HEX, градиенты, decorations, hover, click и вложенные компоненты. pnAuth добавляет к ним безопасное пространство действий `auth`. Строка, содержащая `<auth:...>`, всегда разбирается как MiniMessage даже при другом общем формате, поэтому интерактивное действие одинаково работает на всех поддерживаемых платформах.

Синтаксис действия:

```text
<auth:название_действия>видимый текст или другие MiniMessage-теги</auth>
```

| Действие | Назначение |
|---|---|
| `open_dialog` | Повторно открыть нативное окно регистрации или входа для текущего игрока |

pnAuth самостоятельно преобразует действие в совместимый click event для BungeeCord, Velocity и Paper; внутренняя служебная команда в конфигурации не раскрывается. При неизвестном действии pnAuth выводит в консоль ошибочное имя и список доступных действий, а действие не выполняется.

Полное сообщение об ошибке, включая префикс, причину, hover и кнопку, хранится одной строкой:
...
Плейсхолдер `{error}` заменяется безопасно: содержимое ошибки не интерпретируется как новый MiniMessage-тег. Неизвестные `auth`-действия не выполняются. Для немедленного автоматического повторного открытия формы без сообщения используйте:
...
Во время фактической проверки пароля pnAuth показывает непрерывный title без очистки между кадрами. Стандартный пресет содержит 48 уникальных фаз и обновляется каждые 2 игровых тика. `stay-millis` задаёт время естественного исчезновения title, а не скорость кадра.
...
ui:
  processing-title:
    enabled: true
    animation:
      # NONE — без анимации; GRADIENT — плавные фазы; FRAMES — собственный список кадров.
      type: GRADIENT
      colors: ["#7c3aed", "#a855f7", "#6366f1", "#38bdf8", "#7c3aed"]
      frame-count: 48
      # Для FRAMES каждый элемент поддерживает MiniMessage и плейсхолдер {text}.
      frames: []
    timings:
      # 2 тика = 100 мс. Меньшее значение обновляет title быстрее.
      frame-interval-ticks: 2
      stay-millis: 5000
```

Пользовательские `colors`, `frame-count`, `frames` и timings не заменяются обновлением. Автоматическая миграция применяется только к точным старым стандартным пресетам pnAuth.
...
Изменение безопасности (смена пароля или удаление аккаунта) сначала фиксируется в авторитетной SQL-базе/Hub и немедленно отзывает локальную сессию. Событие синхронизации атомарно сохраняется в каталоге `cluster-outbox` и повторяется до тех пор, пока Redis, Hub или SQL-транспорт не подтвердит запись. Недоступность транспорта не откатывает новый пароль к старому. После смены пароля также очищается trusted-IP session, поэтому повторное подключение всегда требует уже новый пароль. Дубликаты событий безопасны: операции отзыва сессии идемпотентны.
...
Планировщики телепортации совместимы с региональной моделью Folia. Для автономного Paper не нужен proxy-сервер; используется тот же `pnAuth-1.1.0.jar`.

## Limbo

Встроенный PicoLimbo отключён по умолчанию. При включении `limbo.enabled: true` добавьте его `server-name` в `servers.auth`; pnAuth распознает Limbo по совпадению имени автоматически. Поле `type` у маршрутов не требуется. Имена в `servers.backend` должны отличаться от `limbo.server-name`. Бинарный файл загружается только из заданного URL и проверяется SHA-256.
